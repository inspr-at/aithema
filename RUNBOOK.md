...
This package prepares public GitHub-source candidate `0.7.0` (explicit `legacy-semver-public`). Releases `0.1.0` through `0.6.0` are already published at [inspr-at/aithema](https://github.com/inspr-at/aithema) (`v0.1.0` through `v0.6.0`). The commands below are for local operator/developer use. They do not publish to npm, create a tag, deploy, or prove a live provider or OIDC integration.
...
tar -xzf dist/inspr-aithema-core-source-0.7.0/inspr-aithema-core-source-0.7.0.tgz -C /tmp/aithema-source-review
