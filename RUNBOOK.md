...
- Optional configured estimated spend uses `policy.estimatedSpend: { currency, maxMicroPerProject }`. Each allowed provider must declare the same `estimatedSpend.currency` and a `models` entry for each model that may be selected: `{ unit: "tokens", inputMicroPerMillion, outputMicroPerMillion, maxMicroPerCall }`. All amounts and rates are non-negative integer micro-units supplied and reviewed by the operator; Aithema does not discover prices. A project override may lower `maxMicroPerProject` but cannot change currency, disable the budget, or raise it.
